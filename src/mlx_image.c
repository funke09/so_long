...
void    load_image(t_long *so_long, int x, int y)
{
    ...
    mlx_put_image_to_window(so_long->mlx_ptr.mlx, so_long->mlx_ptr.win, so_long->image, x * SIZE, y * SIZE);
...
void    my_mlx(t_long *so_long)
{
	...
	so_long->mlx_ptr.win = mlx_new_window(so_long->mlx_ptr.mlx, g_x * SIZE, g_y * SIZE, "so_long ");
    pos_player(so_long);
